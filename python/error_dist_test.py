...
def penalise_aa_mismatch(x1, x2, dist_matrix, penalty=1000, 
    ...
    if aa_groups_m1!=None:
        ...
        for aa_group in aa_groups_m1:
            penalty_matrix.loc[x1["Res_typem1"].isin(list(aa_group)), 
                            x2["Res_typem1"].isin(list(aa_group))] = 0          
        dist_matrix = dist_matrix + penalty_matrix
...
for id in df_wide["ID"].unique():
    ...
    tmp = pd.DataFrame({"ID":id, "Res_name":dist_pred_HN.index, "N_aa":len(dist_pred_HN.index),
                        ...
                        "nearest_pred_HN_HADAMAC":dist_pred_HN_HADAMAC.min(axis=0),
                        ...
                        "nearest_pred_HNCO_HADAMAC":dist_pred_HNCO_HADAMAC.min(axis=0),
...
def nearest_neighbour_dist_plots():
    ...
    plt_base = plt_base + theme(legend_position=(0.7,0.8)) + xlim(0,5)
    plt_base = plt_base + xlab("Scaled combined shift difference")
    
    for x in ["HN","HNCO","HN_HADAMAC","HNCO_HADAMAC","CA_CO", "most"]:
        plt = plt_base
        plt = plt + geom_density(aes(x="dist_correct_"+x, colour=True)) 
        plt = plt + geom_density(aes(x="nearest_obs_"+x, colour=False))
...
df.loc[df["Atom_type"].isin(["Cm1","CAm1","CBm1"]),"Res_atom"] = (
         df.loc[df["Atom_type"].isin(["Cm1","CAm1","CBm1"]),"Res_typem1"])
...
df2.loc[df2["Atom_type"].isin(["Cm1","CAm1","CBm1"]),"Res_atom"] = (
