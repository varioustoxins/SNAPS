...
seq_df = seq_df[["ID","Res_N","Res_type_obs","Res_type_pred",
                 "Res_typem1_obs","Res_typem1_pred"]]
...
df = df[["ID_Res", "ID","Res_name","Res_N","Res_type","Res_typem1",
...
df2["Res_cor"] = df2["Res_type"]
df2.loc[df2["Atom_type"].isin(["Cm1","CAm1","CBm1"]),"Res_cor"] = (
         df2.loc[df2["Atom_type"].isin(["Cm1","CAm1","CBm1"]),"Res_typem1"])

df2["Delta_cor"] = np.nan
lm_results = pd.DataFrame(columns=["Atom_type","Res_type","Grad","Offset"])

for atom in atom_set:
    for res in df2["Res_cor"].unique():
        mask = (df2["Atom_type"]==atom) & (df2["Res_cor"]==res)
...
plt = plt + facet_wrap("Atom_type", scales="free_y")
plt = plt + xlim(-5,5) + xlab("Prediction error")
plt = plt + ggtitle("Normalised prediction error has a slightly tighter "+
                    "distribution than the original predictions")
plt = plt + scale_linetype_discrete(name="Prediction type")
plt.save(path/"plots/Norm error distribution.pdf", 
         height=210, width=297, units="mm")

plt1 = ggplot(data = df2)
plt1 = plt + geom_point(aes(x="Shift_obs", y="Delta_cor", colour="Res_cor"))
plt1 = plt + facet_wrap("Atom_type", scales="free")
plt1 = plt + xlab("Observed shift") + ylab("Normalised prediction error")
plt1 = plt + ggtitle("Normalised prediction error is independent of observed shift...")

plt2 = ggplot(data = df2)
plt2 = plt + geom_point(aes(x="Shift_obs", y="Delta_cor", colour="Res_cor"))
plt2 = plt + facet_wrap("Atom_type", scales="free")
plt2 = plt + xlab("Observed shift") + ylab("Normalised prediction error")
plt2 = plt + ggtitle("...but is dependent on predicted shift. But I don't think this is a problem.")

save_as_pdf_pages(filename=path/"plots/Norm error vs shift.pdf",
                  plots=[plt1, plt2])


plt = ggplot(df2.loc[df2["Res_cor"]=="Y",:]) 
...
plt = plt + facet_wrap("Atom_type", scales="free")
plt.save(path/"plots/tmp.pdf", height=210, width=297, units="mm")

def dist_comparison_plots():
    for res in df2["Res_cor"].unique():
        plt = ggplot(df2.loc[df2["Res_cor"]==res,:]) 
        plt = plt + geom_density(aes(x="Shift_obs", colour=["Obs"]))
        plt = plt + geom_density(aes(x="Shift_pred", colour=["Pred"]))
        plt = plt + geom_density(aes(x="Shift_pred-Delta+Delta_cor", colour=["Pred_corr"]))
        plt = plt + facet_wrap("Atom_type", scales="free")
